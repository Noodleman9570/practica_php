<?php
    ...
    class HospitList extends Controllers
    {
        ...
        public function HospitList()
        {
            Auth::accessPage();
            
            $data['page_title'] = "Hospital";
            $data['page_name'] = "HospitalizacionList";
            $data['function_js'] = "/hospitList.js";
            $data['style_css'] = "/hospitList.css";
            $this->views->getView($this,"hospitList",$data);
        }
        public function all()
        {
            $arrJson = [];
            $pac = hospitListModel::all();
            if(empty($pac)){
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                //validar
                    ...
                    if($val->isSuccess()){
                        ...
                        $pac = hospitListModel::verifyCed(clear($_POST['cedula']));
                        ...
                        if (!$pac) {
                            ...
                            try {
                                $idInsert = hospitListModel::insert('TMBCH_PAC', $data);
        ...
        {
            $idInsert = hospitListModel::SQL('SELECT MAX(TMPAC_PID) FROM TMBCH_PAC');
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                //validar
                    ...
                    if($val->isSuccess()){
                            ...
                            try {
                                $idInsert = hospitListModel::update('TMBCH_PAC', $data, $ids);
        ...
        {
            $id = intval($_POST['id']);
            $pac = hospitListModel::onePaciente($id);

            if(empty($pac)){
                Alertas::new("No se encontro el usuario", "danger");
                header('Location:'.BASE_URL.'/HospitList');
            }

            hospitListModel::deletePaciente($id);
        ...
        {
            try {
                $consulta = hospitListModel::SQL("SELECT * FROM TMBCH_EDO ORDER BY TMEDO_NO ASC");
            } catch (Exception $e) {
                echo "ERROR: ".$e->getMessage();
            }
        ...
        public function listarMUN()
        {
            try {
                $idedo = $_POST['idedo'];
                $consulta = hospitListModel::SQL("SELECT * FROM TMBCH_MUN WHERE TMEDO_CE = ".$idedo.";");
            } catch (Exception $e) {
                echo "ERROR: ".$e->getMessage();
            }
